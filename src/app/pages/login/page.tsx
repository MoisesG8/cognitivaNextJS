...
export default function Login() {
  ...
  const iniciarSesion = async () => {
    ...
    setCargando(true)
    const respuesta = await myFetch("http://localhost:8080/api/v1/auth/login", "POST", {
      "email": email,
      "password": password
    })
    if (respuesta && respuesta?.token != null) {
      setCookie("auth", respuesta?.token, 1)
      setCookie("user", JSON.stringify(respuesta?.usuarioLoginResponse), 1)
      setAcceso(true)
      setUser(respuesta?.usuarioLoginResponse)
